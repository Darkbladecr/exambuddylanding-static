...
window.vc || (window.vc = {}),
    ...
    function($) {
        ...
        vc.events.on("vc:access:initialize", function(access) {
            ...
            var $buttonsContainerSelector = ($('#nectar-metabox-portfolio-extra').length == 0) ? '#post-body-content > .composer-switch' : '#post-body > .composer-switch';
            ...
            gutenbergEditor = document.getElementById("editor"), $titleDiv.length ? (access.can("fe_editor") && (front = '<a class="wpb_switch-to-front-composer" href="' + $("#wpb-edit-inline").attr("href") + '">' + window.i18nLocale.main_button_title_frontend_editor + "</a>"), access.can("classic_editor") ? access.can("be_editor") && (back = '<a class="wpb_switch-to-composer" href="javascript:;">' + window.i18nLocale.main_button_title_backend_editor + "</a>") : ($("#postdivrich").addClass("vc-disable-editor"), access.can("be_editor") && !vc_user_access().isBlockEditorIsEnabled() && _.defer(function() {
            ...
            })), window.wpbIsGutenberg && (gutenberg = '<a class="wpb_switch-to-gutenberg" href="' + window.wpbGutenbergEditorSWitchUrl + '">' + window.i18nLocale.main_button_title_gutenberg + "</a>"), (front || back || gutenberg) && ($buttonsContainer = $titleDiv.length ? $('<div class="composer-switch"><div class="composer-inner-switch"><span class="logo-icon"></span>' + back + front + "</div>" + gutenberg + "</div>").insertAfter($btnLocation) : $('<div class="composer-switch"><div class="composer-inner-switch"><span class="logo-icon"></span>' + back + front + "</div>" + gutenberg + "</div>").prependTo("#post-body-content"), access.can("classic_editor") && $($buttonsContainerSelector).find(".wpb_switch-to-composer").on("click", function(e) {
                ...
                gutenbergEditorHeader && ($buttonsContainer = $('<div class="composer-switch"><div class="composer-inner-switch"><span class="logo-icon"></span>' + back + "</div></div>").appendTo(gutenbergEditorHeader))
            }, 1)
            /* nectar addition end */
        })
    }(window.jQuery);